...
**Implementation status:** Slices 1–8 are implemented in `evidence-demo/`. Familiarity is measured at `baseRevision` for **modified** files; **added** files use the greenfield gate (`changeKind === 'added'` → `high`). Commits and line changes in `base..head` are excluded from pre-PR signals.

## Measurement boundary

Familiarity answers: **"How familiar was the PR author with this file before this change?"**

For **modified** files, all signals are measured at **`baseRevision`**:
...
Commits and line changes in **`base..head` are excluded** from pre-PR signals. The PR under review must not inflate familiarity — an author's first touch of an **existing** file in this PR should not read as `moderate` merely because the PR commit exists.

**`headRevision`** remains in the changed-files contract for diff resolution and blast-radius analysis only. Familiarity does not use `headRevision` for pre-PR signals.

**Edge cases:**

- **File added in the PR** (`changeKind: 'added'`): path absent at `baseRevision` (or added in the `base..head` diff). Pre-PR signals are zero. Characterization is **`high`** via the greenfield gate — not from line/commit thresholds. Report uses greenfield copy (see LLD 0003); do not lead with "0% of lines before this PR" as if the author were unfamiliar.
- **First touch of an existing file** (`changeKind: 'modified'`, zero pre-PR history): **`none`** — unfamiliar with legacy code others have maintained.
...
**Two kinds of none-adjacent cases.** Both can show zero pre-PR commits and zero line shares, but they mean different things. A **greenfield add** has no prior path — the author is the sole contributor in this change, so the label is **`high`**. A **first touch of an existing file** has substantial history from others — the author is entering unfamiliar territory, so the label is **`none`**. The report must not treat them the same.

## What it computes

For the PR author and each unique changed file path, it derives **change kind**, then three classes of pre-PR signals (for modified files only). Commit counts measure activity and recency; line-level blame measures ownership. **Commit-share is not line ownership** ... counting how many times someone committed to a file is a weak proxy for familiarity when one author can rewrite most of a file in a single commit while another makes many small edits.

### Change kind

Each changed file is classified before characterization:

- **Primary detection:** `git diff --diff-filter=A --name-only base...head` (or equivalent) marks added paths.
- **Fallback / consistency check:** path does not exist at `baseRevision` (`git cat-file -e baseRevision:path` fails).
- **Default:** paths in the changed-files list not marked added are **`modified`**.

v1 does not follow renames (`git diff -M`); a rename may appear as add+delete. See limitations.

### Line-level signals (primary ownership indicators)

For **modified** files only. **Added** files skip blame at `baseRevision` (path absent); all line counts are zero.
...
For **modified** files, from `git log --since=<windowStart> <baseRevision>` over the same six-month window (not through head):
...
For **added** files, all commit fields are zero.

### Characterization

From change kind and (for modified files) all pre-PR signals, it produces a per-file familiarity characterization (`high`, `moderate`, or `none`) and every underlying number that justifies it, because the justification is the point, not the label. There is no directory aggregation ... each finding corresponds to one changed file path, aligned with the changed-files list and blast-radius findings.

**Greenfield gate (short-circuit).** Applied before the combined rule:

| Tier | Rule |
|------|------|
| **high (greenfield)** | `changeKind === 'added'` → `high` (pre-PR signals ignored for labeling; numeric fields remain zero) |

Greenfield `high` means the author is the sole contributor on a new path — not high line ownership on legacy code. Recency gates do not apply (no pre-PR `lastTouchDate`).

**Combined rule (modified files only).** When `changeKind === 'modified'`, characterization uses the stronger of line signals or commit activity, gated by recency. Default thresholds (tunable constants):
...
interface FamiliarityFinding {
  touchedFile: string;
  changeKind: FileChangeKind;
  // Line-level (zero for added files)
  authorOwnedLineCount: number;
  totalBlameableLineCount: number;
  ...
  // Commit-level (zero for added files)
  authorCommitCount: number;
  totalFileCommitCount: number;
  lastTouchDate: Date | null;
...
`ChangedFilesResult.changedFiles` as `ChangedFileEntry[]` is a breaking change from `string[]`; blast-radius and report consumers derive paths from `entry.path`.
...
Slice 8 ... resolve `changeKind` per path in `changedFiles.ts`; add greenfield gate to `characterizeFamiliarity`; greenfield report copy in `renderReport.ts`; fixtures for added-file → `high` vs first-touch-modified → `none`.
...
**Commit-history caveats** ... renamed and moved files (history breaks across renames without follow logic; renames may misclassify as add+delete for change kind), squashed commits (collapse many authors' work into one), bot commits and co-authored commits (mis-attribute authorship), reviewers who shaped code but rarely commit (invisible to commit history), and new team members (no history is not the same as low familiarity in a way the demo will not yet distinguish).

**Blame-specific caveats** ... binary or non-text files (no meaningful line blame), generated or minified files (misleading ownership), rename tracking (`git blame -M` / `-C` not applied ... lines may attribute to the wrong author across moves), squash merges collapsing multi-author line history, `Co-authored-by` trailers not reflected in blame attribution, whitespace-only or formatting-only churn inflating windowed metrics, and files that did not exist at `baseRevision` (zero blameable lines; greenfield copy applies when `changeKind === 'added'`).
...
The report should show the numbers, not just the label. For **modified** files, lead with line facts, then commit facts. Pre-PR numeric copy applies to modified files only.
